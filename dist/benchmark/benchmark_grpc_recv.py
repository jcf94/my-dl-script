# Fork from: https://gist.github.com/yaroslavvb
...
flags = tf.flags
flags.DEFINE_integer("iters", 100, "Maximum number of additions")
flags.DEFINE_integer("data_mb", 512, "size of vector in MBs")
...
flags.DEFINE_string("task", "", "# of current task")
...
def run_benchmark_local(pos1, pos2):
  ops = create_graph(pos1, pos2)
  sess = tf.Session(config=session_config())
  return run_benchmark(sess, *ops)


def run_benchmark_distributed():
  ops = create_graph("/job:worker/task:0/cpu:0", "/job:worker/task:1")
  ...
  def run_worker(w):
    ...
    if FLAGS.profile:
      my_env["LD_PRELOAD"]="/usr/lib64/libtcmalloc_and_profiler.so.4"
...
if __name__=='__main__':
  if not FLAGS.task:
    ...
    #rate1 = run_benchmark_local("CPU:0", "CPU:0")
    rate1 = 0
    ...
    server = tf.train.Server(clusterspec(), config=session_config(),
                             job_name="worker",
                             task_index=int(FLAGS.task), protocol="grpc+verbs")
    ...
    if FLAGS.verbose:
      print("Worker %s quitting." %(FLAGS.task))
